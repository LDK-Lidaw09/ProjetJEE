...
public class ViewClient extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ViewClient</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ViewClient at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
   ...
   ClientDao dao = new ClientDao();
    ...
    out.print("<tr>"
        ...
        out.print("<tr><td>"+e.getIdClient()+"</td><td>"+e.getNomClient()+"</td><td>"+e.getTypeClient()+"</td><td>"+e.getAdresseClient()+"</td><td>"+e.getSpecialiteClient()+"</td><td>"+e.getLogin()+"</td><td>"+e.getPassword()+"</td><td>"+e.getProfil()+"</td>"
            + "<td><a class=\"btn btn-success mt-3\" href='EditClient?id="+e.getIdClient()+"'>edit</a></td>"
                + "<td><a class=\"btn btn-danger mt-3\" href='DeleteClient?id="+e.getIdClient()+"'>delete</a></td>"
                    + "</tr>");
        }
        out.print("</table>");
        out.println("<a class=\"btn btn-success mt-3\"  href='Client/form.jsp'>Add New Client</a>");
        out.close();
    }
